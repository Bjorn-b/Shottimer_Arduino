...
void startTimer() {
  ...
  lcd.setCursor(0,0); lcd.print(F("Wait for it...  "));
  lcd.setCursor(0,1); lcd.print(F("                ")); // create a clearline function? Save fewer strings in progmem?
  startDelay();
  lcd.setCursor(0,0); lcd.print(F("      GO!!      "));
  lcd.setCursor(0,1); lcd.print(F("Last:")); //10 chars
  BEEP(beepNote, beepDur, beepTone);
  shotTimer.start();
...
void stopTimer(boolean out = 0) {
  ...
  shotTimer.stop();
  Serial.println(F("Timer was stopped at:")); serialPrint(shotTimer.elapsed(), 7);
  for (int i = 0; i < 5; i++){
    BEEP(NOTE_C5, 100, beepTone);
...
void reviewShots(){
  ...
  if (reviewingShots == 1){
     if (currentShot > 0) {reviewShot = currentShot - 1; } Serial.print(reviewShot);
     for (int t = 0; t < currentShot; t++){ 
       Serial.print(F("Shot #")); Serial.print(t + 1); Serial.print(F(" - "));
       serialPrint(shotTimes[t],7); 
     }
     //lcd.setBacklight(VIOLET);
     lcd.setCursor(0,0);
     lcd.print(F("Shot #")); lcd.print(currentShot); lcd.print(F("                ")); lcd.setCursor(9,0); lcd.print(F(" Split "));
     lcd.setCursor(0,1);
     lcdPrint(shotTimes[reviewShot],7); lcd.print(F(" ")); if(reviewShot > 1) {lcdPrint(shotTimes[reviewShot]-shotTimes[reviewShot-1],5);}
                     //9 characters             //1 characters                    //6 characters
  } else {
    //lcd.setBacklight(WHITE);
    returnToMenu();
...
void nextShot(){
  ...
  lcd.setCursor(0,0);
  lcd.print(F("Shot #")); lcd.print(reviewShot + 1); lcd.print(F("                ")); lcd.setCursor(9,0); lcd.print(F(" Split "));
  lcd.setCursor(0,1); 
  lcdPrint(shotTimes[reviewShot],7); lcd.print(F(" ")); if(reviewShot == 0) {lcd.print(F("   1st"));} else {lcdPrint(shotTimes[reviewShot]-shotTimes[reviewShot-1],5);}
}
...
void rateOfFire(boolean includeDraw = true){
 ...
 lcd.print(F("                "));
 lcd.setCursor(0,0);
 lcd.print(F("Avg Split:")); lcd.setCursor(11,0); lcdPrint(rof,5);
 lcd.setCursor(0,1);
 lcd.print(F("                "));
 lcd.setCursor(0,1);
 lcd.print(F("Shots/min:")); lcd.setCursor(11,1); lcd.print(60000/rof); // will this produce a decimal? Or a truncated int? 
...
void setEchoProtect(){
 ...
 if (settingEcho == 1){
     ...
     lcd.setCursor(0,1);
     lcd.print(sampleWindow); lcd.print(F("ms"));  
 } 
 else { 
...
void setParTimes(){
 ...
 if (settingParTimes == 1){
     lcd.clear();
     lcd.setCursor(0,0);
     lcd.print(F("<<"));lcd.setCursor(5,0); lcd.print(F("Par"));
     lcd.setCursor(9,0); lcd2digits(currentPar + 1);
     lcd.setCursor(4,1); if (currentPar > 0){lcd.print(F("+")); } else {lcd.print(F(" "));}
...
void editPar(){
 ...
 if (editingPar == 1){
     //lcd.setBacklight(GREEN);
     lcd.setCursor(0,0); lcd.print(F("Edit        "));
     lcd.setCursor(0,1); lcd.print(F("P")); lcd2digits(currentPar + 1); if (currentPar > 0){lcd.print(F(" +"));} else {lcd.print(F("  "));}
...
void BEEP(int Note, int Duration, int Tone){
...
void menuChangeEvent(MenuChangeEvent changed)
{
  ...
  lcd.setCursor(0,1);
  lcd.print(changed.to.getName()); lcd.print(F("            ")); //12 spaces
}
...
void button_click_actions() {
  BEEP(buttonNote, buttonDur, buttonTone);
...
// debugAudioInput();
  ...
  uint8_t newButtons = read_LCD_buttons();
    ...
    if (parEnabled == 1){
      ...
      for (byte i = 0; i < parLimit; i++){
        ...
        if (shotTimer.elapsed() <= (additivePar + (sampleWindow / 2)) && shotTimer.elapsed() >= (additivePar - sampleWindow / 2)){
         BEEP(beepNote, beepDur, beepTone);  //Beep if the current time matches (within the boundaries of sample window) the parTime
        }
      }
  ...
  if (buttons){  
      if (shotTimer.isRunning()){ //while timer is running
          ...
          if (buttons & BUTTON_UP) {
            //buttonTone();
            Serial.println(F("BUTTON_UP"));
            menu.moveUp();
            freeRAM();
          }
          if (buttons & BUTTON_DOWN) {
            //buttonTone();
            Serial.println(F("BUTTON_DOWN"));
            menu.moveDown();
            freeRAM();
          }
          if (buttons & BUTTON_LEFT) {
            //buttonTone();
            Serial.println(F("BUTTON_LEFT"));
            menu.moveLeft();
            freeRAM();
          }
          if (buttons & BUTTON_RIGHT) {
